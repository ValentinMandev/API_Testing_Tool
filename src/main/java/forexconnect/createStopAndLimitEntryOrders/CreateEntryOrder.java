...
//        }
    ...
    private static void printSampleParams(String procName,
        ...
        if (prm != null) {
            System.out.println(String.format("Instrument='%s', BuySell='%s', Lots='%s', AccountID='%s', ExpireDate='%s'",
                    prm.getInstrument(), prm.getBuySell(), prm.getLots(), prm.getAccountID(), prm.getExpireDate()));
        }
    }
}
...
public class MyApp extends MessageCracker implements Application {
    ...
    private static String outputFile = "output/fixapi/orders_data.txt";
    ...
    private Set<Map<Integer, List<String>>> marketData;
    ...
    public Set<Account> getAccts() {
        return new LinkedHashSet<Account>(accounts.keySet());
    }

    public Set<OrderID> getOrdersPlaced() {
        return new LinkedHashSet<OrderID>(orders.keySet());
    }

    public Set<String> getPositionsExecuted() {
        return new LinkedHashSet<String>(positions.keySet());
    }
    ...
    public Set<String> getInstruments() {
        return new LinkedHashSet<String>(instruments.keySet());
    }

    public PositionReport getPositionReport(String ticketID) {
        return positions.get(ticketID);
    }
...
//END SECTION - public trading functions
    ...
    public MyApp(SessionSettings settings, Set<Map<Integer, List<String>>> marketData) {
    ...
    public void sendMarketDataRequest(char aSubscriptionRequestType, List<String> instruments) {
        try {
            SubscriptionRequestType subReqType = new SubscriptionRequestType(aSubscriptionRequestType);
            MarketDataRequest mdr = new MarketDataRequest();
            mdr.set(new MDReqID(String.valueOf(nextID())));
            ...
            for (int i = 1; i <= max; i++) {
                ...
                if (instruments.contains(group.getInstrument().getSymbol().getValue())) {
                    MarketDataRequest.NoRelatedSym symbol = new MarketDataRequest.NoRelatedSym();
                    symbol.set(group.getInstrument());
                    mdr.addGroup(symbol);
                    SecurityStatusReqID id = new SecurityStatusReqID(String.valueOf(nextID()));
    ...
    public void sendEntryOrder(Account account, Side side, String symbol, OrdType ordType, double price)
            throws FieldNotFound {
        // set up a multiplier for the lot size
        int lotValue = 1000;
        // if the symbol to be traded is the US Dollar, the multiplier should be 1
        if (symbol.equalsIgnoreCase("USDOLLAR")) lotValue = 1;
        // send the market order
        sendEntryOrder(sessionID, accounts.get(account), side, instruments.get(symbol).getSymbol(), ordType, price,
                new OrderQty(instruments.get(symbol).getDouble(FXCMMinQuantity) * lotValue),
                new TimeInForce(TimeInForce.GOOD_TILL_CANCEL));
    }

    public void sendEntryOrder(SessionID sessionID, CollateralReport account,
                               Side side, Symbol symbol, OrdType ordType, double price, OrderQty orderQty, TimeInForce timeInForce)
            throws FieldNotFound {
        // create a new order with an new temporary id, Side, new transaction time and set type as OrdType.MARKET
        NewOrderSingle order = new NewOrderSingle(
                new ClOrdID(sessionID + "-" + System.currentTimeMillis() + "-" + Long.toString(nextID())),
...
//            System.out.println(snapshot.getInstrument().getSymbol().getValue());
//            System.out.println(snapshot);

            Map<Integer, List<String>> messageToMap = new HashMap<>();
            Arrays.stream(snapshot.toString().split("\u0001")).forEach(tag -> {
                String[] data = tag.split("=");
                messageToMap.putIfAbsent(Integer.parseInt(data[0]), new ArrayList<>());
                messageToMap.get(Integer.parseInt(data[0])).add(data[1]);
            });

            marketData.add(messageToMap);

//            Arrays.stream(snapshot.toString().split("\u0001")).forEach(System.out::println);
//            System.exit(0);
    ...
    public void onMessage(ExecutionReport report, SessionID sessionID)
        ...
        Price price = report.getPrice();
        OrderQty quantity = report.getOrderQty();
        OrdType ordType = report.getOrdType();
        Side side = report.getSide();
        TimeInForce tif = report.getTimeInForce();
        OrdStatus status = report.getOrdStatus();

        String result = String.format("Symbol: %s, price: %.5f, quantity: %.0f, order type: %s, side: %s, time in force: %s, status: %s",
                symbol.getValue(), price.getValue(), quantity.getValue(), ordType.getValue(), side.getValue(), tif.getValue(), status.getValue());
    ...
    public void sendOrdersRequest() {
        ...
        request.set(new MassStatusReqID("PendingOrdersRequest"));
        request.set(new MassStatusReqType(7));
        requestCompleted = false;
        // send the order to the api
